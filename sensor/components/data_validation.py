...
class DataValidation:
    ...
    def drop_missing_values_columns(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:
        ...
        ==========================================================================================
        return Pandas DataFrame or if atleast single column is available after missing values column
        drop
        """
        try:
            ...
            #Selecting Columns name which contains null value more than threshold 
            logging.info(f"Selecting Columns name which contains null value more than threshold {threshold}")
    ...
    def data_drift(self, base_df:pd.DataFrame, current_df:pd.DataFrame,report_key_name:str):
        try:
            ...
            for base_column in base_columns:
                ...
                #Null hypothisis is that both column data drawn from same distribution

                logging.info(f"Hypothisis {base_column}: {base_data.dtype},{current_data.dtype}")
                same_distribustion = ks_2samp(base_data,current_data)

                if same_distribustion.pvalue>0.05:
                    #We are accepting null hypothesis
                    drift_report[base_column]={
                        "pvalues":float(same_distribustion.pvalue),
    ...
    def initiate_data_validation(self)-> artifact_entity.DataValidationArtifact:
        ...
        current_df=train_df, report_key_name="misisng_columns_within_train_dataset")
        ...
        current_df=test_df,report_key_name="misisng_columns_within_test_dataset")
