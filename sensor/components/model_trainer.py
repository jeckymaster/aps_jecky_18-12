...
class ModelTrainer:
    ...
    def fine_tune(self):
        try:
            #Need to write code for fine tune such as gridsearch cv or etc
            pass
        except Exception as e:
            raise SensorException(e, sys)
        ...
        try:
            ...
            if f1_test_score < self.model_trainer_config.expected_score:
                raise Exception(f"Model is not good as it is not able to give \
                    expected accuracy: {self.model_trainer_config.expected_score}: model actial score : {f1_test_sore}")
